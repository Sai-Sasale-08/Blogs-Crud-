...
const BlogList = () => {
    ...
    const getblog=()=>{
        axios.get("http://localhost:8080/blog/allblogs",{withCredentials:true})
        .then((res)=>{
    ...
    <div>
        {data.map((el)=>(
            ...
            <Button variant="primary" className="mt-3">
                <Link to={`/update/${el._id}`} style={{color:"black",textDecoration:"none"}}>Update</Link>
            </Button>

            <Button variant="warning" className="mt-3">
                <Link to={`/information/${el._id}`} style={{color:"black",textDecoration:"none"}}>View</Link>
            </Button>
